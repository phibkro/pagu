...
1. **Composable agent loops — iterative review / multi-agent.** Substrate (v1)
   **shipped** 2026-05-28 (`src/loop.ts`): the turn is a `Step<C>`, `runTask` is
   `loop(turn)`. Remaining: `fanOut` (eager-parallel fold of the `Flow` monoid;
   designed 2026-05-29 — race-freedom is a call-site responsibility via
   read-only carriers like `ReadonlyExec`, _not_ a global immutable-carrier
   rewrite) and multi-agent _later_, the point where "independent actors"
...
| **`fanOut` combinator**                        | The loop substrate (`Step<C>`, `loop`, `andThen`, `pipeline`) is not a complete API without `fanOut` (eager-parallel fold of the `Flow` monoid, dual to `pipeline`'s lazy-sequential fold); freezing the surface before this would be premature. Designed 2026-05-29. | open                                                                              |
