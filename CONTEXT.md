...
| **`fanOut` combinator**                        | The loop substrate (`Step<C>`, `loop`, `andThen`, `pipeline`) is not a complete API without `fanOut` (monoidal product for parallel execution); freezing the surface before this would be premature | open   |
| **Config-driven pluggability**                 | Insert custom handlers; the point where the gate-never-widen law is type-enforced and SDK consumers can extend safely. Requires `ReadonlyExec` (layer 2 of gate-never-widen)                        | open   |
| **`ReadonlyExec` — gate-never-widen layer 2**  | Prerequisite for config-driven pluggability; makes terminal handlers provably non-widening at the type level                                                                                        | open   |
