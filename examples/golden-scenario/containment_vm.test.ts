...
import { detectVM } from "../../src/vm/index.ts";
...
//
// First cut covers the DESTRUCTION run (it actually executes in-guest, proving
// pagu runs containerized + the blast radius is the mount + recoverable). The
// exfil/concealment run waits on the launcher's mount-layer masking (the
// in-guest tier-2 replacement — bwrap can't nest in rootless Podman).
